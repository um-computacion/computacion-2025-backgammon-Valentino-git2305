...
class CLI:
    ...
    def _board_ascii(self) -> str:
        ...
        top_indices = list(range(12, 24))         # 12..23
        bot_indices = list(range(11, -1, -1))     # 11..0

        top = " ".join(f"{self._checker_cell(i):>2}" for i in top_indices)
        bot = " ".join(f"{self._checker_cell(i):>2}" for i in bot_indices)
        sep = "-" * max(len(top), 40)
    ...
    # Sintaxis
       ...
       try:
           ...
           print("El dado debe ser un número entre 1 y 6.")
           return
