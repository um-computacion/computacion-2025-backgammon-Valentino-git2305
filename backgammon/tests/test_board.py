# backgammon/tests/test_board.py
...
def owner_at(board, idx):
    """Devuelve la lista de dueños en un punto (de abajo hacia arriba)."""
...
class TestBoard(unittest.TestCase):
    ...
    def test_init_board(self):
        tablero = Board()
        self.assertEqual(len(tablero.__points__), 24)
        for punto in tablero.__points__:
            self.assertIsInstance(punto, list)
        self.assertEqual(tablero.__bar__[Player.WHITE], [])
        self.assertEqual(tablero.__bar__[Player.BLACK], [])
        self.assertEqual(tablero.__borne__[Player.WHITE], [])
    ...
    def test_ascii_contains_bar_and_borne_and_labels(self):
        ...
        # ahora esperamos 'BAR:' y 'BORNE:' exactamente
